...
_MAIN_URL = "https://github.com/facebookresearch/mlqe/tree/main/data"
...
class MlqeDaConfig(datasets.BuilderConfig):
    def __init__(self, src_lg, tgt_lg, **kwargs):
        super(MlqeDaConfig, self).__init__(**kwargs)
        self.src_lg = src_lg
        self.tgt_lg = tgt_lg


class MlqeDa(datasets.GeneratorBasedBuilder):

    BUILDER_CONFIGS = [
        MlqeDaConfig(name=f"{src_lg}-{tgt_lg}", version=datasets.Version("1.1.0"), description=f"Task 1: {src_lg} - {tgt_lg}", src_lg=src_lg, tgt_lg=tgt_lg,)
        for (src_lg, tgt_lg) in _LANGUAGE_PAIRS
    ]
    BUILDER_CONFIG_CLASS = MlqeDaConfig
    ...
    def _info(self):
        ...
        return datasets.DatasetInfo(
            description=_DESCRIPTION, features=features, supervised_keys=None, homepage=_HOMEPAGE, license=_LICENSE, citation=_CITATION,
        )
    ...
    def _generate_examples(self, filepath, split, source_lg, target_lg, files):
        ...
        for path, f in files:
            ...
            if all(x is not None for x in [docids, nmt_outputs, word_probas, translations]):
                for id_, (row_, docid_, nmt_out_, word_prob_) in enumerate(zip(translations, docids, nmt_outputs, word_probas)):
                    yield id_, {
                        "segid": row_[0],
                        "translation": {source_lg: row_[1], target_lg: row_[2]},
                        "scores": [] if (split == "test20" and (source_lg, target_lg) == ("ru", "en")) else row_[3].strip("][").split(", "),
                        "mean": -10_000 if (split == "test20" and (source_lg, target_lg) == ("ru", "en")) else row_[4],
                        "z_scores": [] if (split == "test20" and (source_lg, target_lg) == ("ru", "en")) else row_[5].strip("][").split(", "),
                        "z_mean": -10_000 if (split == "test20" and (source_lg, target_lg) == ("ru", "en")) else row_[6],
                        "model_score": -10_000 if (split == "test20" and (source_lg, target_lg) == ("ru", "en")) else row_[7],
                        "doc_id": docid_,
                        "nmt_output": nmt_out_,
